<?php
include('guard-up-delete.php');

if(isset($_GET['edit'])){
    $id = $_GET['edit'];
    echo $id;
    $rec = mysqli_query($connection, "SELECT * FROM guard WHERE Guard_ID='$id';");
    $record=mysqli_fetch_array($rec);
    
$name=$record['Guard_name'];
$shift=$record['shift'];
$blockid=$record['B_ID'];
...
<!DOCTYPE html>
<html lang="en">
<title>Gaurd Informatiion</title>
...
<header role="banner">
  ...
  <!-- Header2 -->
      ...
      <div class="w3-display-container w3-text-blue" style="font-size: 50px">
          ...
          <div class="w3-display-middle w3-large" > <h1 >Update Gaurd Informatiion</h1></div>
      </div>
      <div class="w3-container" style="padding: 50px 200px 20px 200px">
      
      
      <form class="w3-container" action="g-update.php" method="post">
          
                 <p>
                      <input class="w3-input w3-light-grey w3-animate-input" type="hidden" name="guardid" value="<?php echo $id;?>"></p>
                 <p>
                     <label>Name</label>
